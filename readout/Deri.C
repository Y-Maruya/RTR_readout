...
int main(int argc, char* argv[]){
   if(argc != 3){
      std::cout << "Usage: " << argv[0] << " <tree_file> <config file name>" << std::endl;
      return 1;
   }
   config_data config;
   config.read_config(std::string(argv[2]));
   std::string tree_file = std::string(argv[1]);
   Deri *d = new Deri(tree_file, config.fCAEN);
   d->Loop(config);
   return 0;
}