...
void val(string filename = "laser_5kHz30p8_HV56p22"){
  ...
  for(int i=0;i<64;i++){
    ...
    for(int j=0;j<25;j++){
      ...
      if(hist[i]->GetMaximumBin()-(Max_bin+j*13)>3){
    ...
    hist[i]->Fit("f1","0M");
    double n_photons[3] ={0,1,2};
    double n_photons_gosa[3] ={0,0,0};
    double mean[3] ={f1->GetParameter(2),f1->GetParameter(5),f1->GetParameter(8)};
    double mean_gosa[3] ={f1->GetParError(2),f1->GetParError(5),f1->GetParError(8)};
    ...
    c1->cd(2);
    TF1*pol = new TF1("pol","pol1",0,3);
    TGraphErrors *g1 = new TGraphErrors(3,n_photons,mean,n_photons_gosa,mean_gosa);
    ...
    sigma1[i]=f1->GetParameter(4);
    sigma1err[i]=f1->GetParError(4);
    if(second_Max_bin==0 && third_Max_bin==0){
